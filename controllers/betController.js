...
// --- NEW VALIDATOR for Multi-Bets ---
exports.validatePlaceMultiBet = [
  body('stake').isFloat({ gt: 0 }).withMessage("A positive stake is required.").toFloat(),
  body('selections').isArray({ min: 2, max: 10 }).withMessage("A multi-bet must contain between 2 and 10 selections."),
  
  // Validate each object within the selections array
  body('selections.*.gameId').isMongoId().withMessage("Each selection must have a valid gameId."),
  body('selections.*.outcome').isIn(['A', 'B', 'Draw']).withMessage("Each selection outcome must be 'A', 'B', or 'Draw'."),
  ...
  body('selections').custom(selections => {
      ...
      const uniqueGameIds = new Set(gameIds);
      if (uniqueGameIds.size !== gameIds.length) {
          throw new Error("A multi-bet cannot contain multiple selections from the same game.");
      }
      return true;
  }),
];
...
exports.placeMultiBet = async (req, res, next) => {
  ...
  try {
      ...
      const gameIds = selections.map(s => s.gameId);
      const games = await Game.find({ '_id': { $in: gameIds }, status: 'upcoming' }).session(session);

      if (games.length !== selections.length) {
          throw new Error("One or more selected games are not available for betting (they may have started or do not exist).");
      }
      ...
      for (const selection of selections) {
          ...
          if (!selectionOdds) throw new Error(`Odds for the selected outcome in game ${game.homeTeam} vs ${game.awayTeam} are not available.`);
      ...
      await session.commitTransaction();
      res.status(201).json({ msg: "Multi-bet placed successfully!", bet: multiBet, walletBalance: user.walletBalance });
